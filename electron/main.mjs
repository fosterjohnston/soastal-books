...
function isLiveWorkbookPath(filePath) {
  const n = normalize(String(filePath || ''))
  return n.includes(LIVE_NAME) || n.includes('acounting%20spreadshseet.xlsx')
}
...
function writeFileSafe(filePath, data) {
  ...
  if (path.basename(filePath).toLowerCase() === LIVE_NAME) {
    throw new Error(
      "Refused: Keith's live workbook Documents/Finance/Acounting spreadshseet.xlsx is denylisted.",
    )
  }
